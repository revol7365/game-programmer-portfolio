...
public:
    ...
    bool GetIsWillDelete() const { return mWillDelete.load(); }
    bool GetIsLoginedIn() const { return mIsLoggedIn.load(); }
    ...
    void SetWillDelete() { mWillDelete.store(true); }
    // 로그인 진입은 반드시 OnLogin() 을 사용할 것 (mLastActiveTime 동기 갱신 필요)
    void SetIsLoggedout() { mIsLoggedIn.store(false); }
