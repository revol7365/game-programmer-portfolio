...
public:
	...
	{
		None = 0,
		PeerClose,         // cbTransferred == 0 (정상 FIN)
		ClientForceClose,  // GQCS ret == FALSE (클라이언트 강제 종료, RST 등)
		InternalError,     // IOCP 등록 실패 등 서버 내부 에러
		RecvError,
		SendError,
		RingBufferFull,
		PacketError,
	...
	bool DisConnect(const uint64_t sessionID, DisconnectReason reason = DisconnectReason::None);
	...
	// 크래시 시 디스코드 알림에 포함할 상태 정보
	static void CrashInfoCallback(char* outBuf, int bufSize);
	static BaseServer* _Instance;
