...
void BaseServer::CrashInfoCallback(char* outBuf, int bufSize)
{
    ...
    sprintf_s(outBuf, bufSize,
        ...
        "DC PeerClose:%ld GqcsFail:%ld RecvErr:%ld SendErr:%ld RingBufFull:%ld\\n"
        "DC Protocol:%ld Kick:%ld Shutdown:%ld Logic:%ld PktErr:%ld Internal:%ld\\n"
        "SessionFull: %ld\\n"
        "```",
        ...
        s->mDisconnectCount[static_cast<int>(DisconnectReason::GqcsFail)].load(),
        ...
        s->mDisconnectCount[static_cast<int>(DisconnectReason::ProtocolError)].load(),
        s->mDisconnectCount[static_cast<int>(DisconnectReason::Kick)].load(),
        s->mDisconnectCount[static_cast<int>(DisconnectReason::ServerShutdown)].load(),
        s->mDisconnectCount[static_cast<int>(DisconnectReason::Logic)].load(),
...
void BaseServer::init()
{
    ...
    // Note: accept/worker 스레드 가동은 Start() 에서 수행한다.
    // 서브클래스(예: GroupServer)가 자기 셋업(그룹 매니저, 컨텐츠 스레드 등) 끝낸 뒤
    // Start() 를 호출하도록 하기 위함.
}

bool BaseServer::Start()
{
    if (mListenSock == INVALID_SOCKET) return false;

    // Worker 먼저 띄워서 IOCP 처리가 즉시 가능하게 한 뒤 Accept 시작
    for (int i = 0; i < mWorkerThreadCount; i++)
    {
...
bool BaseServer::DisConnect(const uint64_t sessionID)
{
    // 세션 ID 검증
    ...
    requestRelease(session, DisconnectReason::Logic);
    decreaseSessionIoCount(session);
    return true;
}
...
bool BaseServer::postSend(Session* session)
{
    ...
    if (ret == SOCKET_ERROR)
    {
        ...
        if (err != 10054)
        {
            LOG(L"WSASend Error: %d, sessionID: %llu \n", err, session->id);
        }
...
void BaseServer::requestRelease(Session* session, DisconnectReason reason)
{
    ...
    // pending IO(WSARecv/WSASend) 즉시 취소하여 ioCount 가 빨리 0 으로 떨어지게 함.
    // closesocket 은 releaseSession 에서 수행 — 여기서는 IO 만 취소.
...
void BaseServer::acceptThread()
{
    ...
    while (!bTerminate) {
        ...
        if (ret == SOCKET_ERROR) {
            LOG(L"[Accept] setsockopt SO_LINGER Failed - closing socket. err: %d\n", WSAGetLastError());
            closesocket(clientSock);
            continue;
        }
        ...
        if (newSession == nullptr)
        {
            ...
            LOG(L"[SessionFull] Session pool exhausted, connection rejected. total:%ld\n",
                mSessionFullCount.load());
            closesocket(clientSock);
            continue;
        }
        ...
        if (CreateIoCompletionPort((HANDLE)clientSock, mWorkerThreadHandle, (ULONG_PTR)newSession, 0) == NULL)
        {
            ...
            requestRelease(newSession, DisconnectReason::GqcsFail);
            continue;
        }
        ...
        {
            LOG(L"[Accept] postRecv FAILED \n");
        }

        // (이전: 세션 캡 근처에서 Sleep(1) 쓰로틀 → burst 중 listen backlog 오버플로 유발)
        // createSession() 자체가 nullptr 반환으로 캡 보호하므로 별도 yield 불필요.
    }
}
...
void BaseServer::workerThread()
{
    ...
    while (1) {
        ...
        if (ret == FALSE) {
            ...
            // LOG(L"[GQCS Error] err: %d \n", err);
            requestRelease(session, DisconnectReason::GqcsFail);
            decreaseSessionIoCount(session);
            continue;
        }
        ...
        if (pOv->type == EOperation::RECV && cbTransferred == 0) {
            // 대량 동시 FIN 시 메시지당 로그가 워커 직렬화 시켜 cleanup 지연됨 → 로그 제거.
        ...
        case EOperation::RECV:
        {
            ...
            while (true) {
                ...
                if (dataLen > Packet::BUFFER_DEFAULT - 5) {
                    LOG(L"[ATTACK] Payload exceeds packet buffer sessionID:%llu dataLen:%d max:%d",
                        session->id, dataLen, Packet::BUFFER_DEFAULT - 5);
                    parseError = true;
                    break;
                }
                ...
                if (totalPacketSize > session->recvRingBuffer.GetBufferSize()) {
                    LOG(L"[ATTACK] Oversize packet sessionID:%llu totalSize:%d bufferSize:%d",
                        session->id, totalPacketSize, session->recvRingBuffer.GetBufferSize());
                    parseError = true;
                    break;
                }
...
void BaseServer::ServerControll()
{
    ...
    if (_kbhit())
    {
        ...
        case L'Q':
            ...
            CrashDump::WriteMiniDumpNowOnce(L"Q-key shutdown");
            bTerminate = true;
            break;
...
void BaseServer::LogMonitor()
{
    ...
    // DisconnectReason별 누적 카운터
    LOG(L"[Monitor] DC | PeerClose:%ld GqcsFail:%ld RecvErr:%ld SendErr:%ld RingBufFull:%ld",
        mDisconnectCount[static_cast<int>(DisconnectReason::PeerClose)].load(),
        mDisconnectCount[static_cast<int>(DisconnectReason::GqcsFail)].load(),
    ...
    LOG(L"[Monitor] DC | Protocol:%ld Kick:%ld Shutdown:%ld Logic:%ld PktErr:%ld Internal:%ld",
        mDisconnectCount[static_cast<int>(DisconnectReason::ProtocolError)].load(),
        mDisconnectCount[static_cast<int>(DisconnectReason::Kick)].load(),
        mDisconnectCount[static_cast<int>(DisconnectReason::ServerShutdown)].load(),
        mDisconnectCount[static_cast<int>(DisconnectReason::Logic)].load(),
