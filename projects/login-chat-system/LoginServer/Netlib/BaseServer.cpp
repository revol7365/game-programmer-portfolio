...
void BaseServer::acceptThread()
{
    ...
    while (!bTerminate) {
        ...
        {
            LOG(L"Failed to create session.\n");
            closesocket(clientSock);
            continue;
        }
...
void BaseServer::workerThread()
{
    ...
    while (1) {
        ...
        if (ret == FALSE) {
            ...
            // err=64(ERROR_NETNAME_DELETED)는 상대방 RST로 인한 정상 종료라 로그 생략
            if (err != ERROR_NETNAME_DELETED) {
                LOG(L"[GQCS Error] err=%d, sessionID=%llu, transferred=%lu", err, session->id, cbTransferred);
            }
...
void BaseServer::PrintMonitor()
{
    ...
    ConsolePrintLine(L"======================= Login SERVER %d ========================", DEFAULT_PORT);
    ConsolePrintLine(L"  Time    : %s", buffer);
    ...
    ConsolePrintLine(L"-------------------------------------------------------------------");
    ConsolePrintLine(L"  DC(Client) PeerClose:%-6ld GqcsFail:%-6ld",
    ...
    ConsolePrintLine(L"  DC(Server) RecvErr:%-5ld SendErr:%-5ld RingBuf:%-5ld PktErr:%-5ld SendBuf:%-5ld",
...
void BaseServer::LogMonitor()
{
    ...
    LOG(L"[Monitor] DC(Client) | PeerClose:%ld GqcsFail:%ld",
        mDisconnectCount[static_cast<int>(DisconnectReason::PeerClose)].load(),
        mDisconnectCount[static_cast<int>(DisconnectReason::GqcsFail)].load());
    LOG(L"[Monitor] DC(Server) | RecvErr:%ld SendErr:%ld RingBufFull:%ld PktErr:%ld SendBufFull:%ld",
        mDisconnectCount[static_cast<int>(DisconnectReason::RecvError)].load(),
        mDisconnectCount[static_cast<int>(DisconnectReason::SendError)].load(),
        mDisconnectCount[static_cast<int>(DisconnectReason::RingBufferFull)].load(),
        mDisconnectCount[static_cast<int>(DisconnectReason::PacketError)].load(),
